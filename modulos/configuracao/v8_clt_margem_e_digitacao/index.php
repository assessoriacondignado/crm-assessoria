...
<div class="modal-body bg-light p-4">
    <form id="formChaveV8">
        ...
        <div class="row g-3 mb-4">
            ...
            <div class="col-md-3"><label class="small fw-bold text-info">Prazo Padrão (Ex: 24, 36, 48):</label><input type="number" id="chv_prazo_padrao" class="form-control border-info" value="24" required></div>
            <div class="col-md-3"><label class="small fw-bold text-info">Averbadora:</label><input type="text" id="chv_averbadora" class="form-control border-info" value="QI" placeholder="Ex: QI, CELCOIN"></div>
            <div class="col-md-12">
                <label class="small fw-bold text-warning">⏱️ Intervalo entre Consentimentos (segundos):</label>
                <input type="number" id="chv_intervalo_consentimento" class="form-control border-warning" value="0" min="0" placeholder="0 = sem pausa (padrão)">
                <small class="text-muted">Tempo de espera entre cada solicitação de consentimento no lote. 0 = sem pausa.</small>
            </div>
        </div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        v8CarregarCadastros(); v8CarregarDadosIniciais().then(() => {
            // Deep-link vindo do Hub: ?tab=lote&chave_id=X
            const _p = new URLSearchParams(window.location.search);
            if (_p.get('tab') === 'lote') {
                const btnLote = document.querySelector('[data-bs-target="#tab-lote-csv"]');
                if (btnLote) btnLote.click();
                const _cid = _p.get('chave_id');
                if (_cid) {
                    const _sel = document.getElementById('lote_v8_chave');
                    if (_sel) { _sel.value = _cid; v8AtualizarSaldosTopo(_cid); }
                }
            }
        }); v8CarregarFila(0);
    ...
    async function v8CarregarClientes() { const tb = document.getElementById("v8_tabela_clientes"); tb.innerHTML = '<tr><td colspan="6" class="text-center py-4"><i class="fas fa-spinner fa-spin"></i></td></tr>'; const thead = document.getElementById("v8_thead_clientes"); let htmlHeader = `<tr><th>Cliente / Credencial</th><th>Usuário ID (Dono)</th>`; if (!restricaoCustoCliente) htmlHeader += `<th>Custo p/ Cliente</th>`; if (!restricaoCustoApi) htmlHeader += `<th>Seu Custo API</th>`; htmlHeader += `<th>Saldo Atual</th><th class="text-center">Status</th><th class="text-center">Ações</th></tr>`; thead.innerHTML = htmlHeader; const r = await v8Req('v8_api.ajax.php', 'listar_chaves_acesso', {}, false); if (r.success) { tb.innerHTML = ''; if (r.data.length === 0) return tb.innerHTML = '<tr><td colspan="7" class="text-center py-4 fw-bold">Nenhuma chave encontrada.</td></tr>'; r.data.forEach(c => { const ativo = c.STATUS === 'ATIVO'; let tr = `<tr class="border-bottom border-dark ${ativo ? '' : 'table-secondary text-muted'}"><td class="fw-bold ${ativo ? 'text-primary' : 'text-muted'}">${c.CLIENTE_NOME}</td><td>${c.NOME_USUARIO}</td>`; if (!restricaoCustoCliente) tr += `<td>R$ ${parseFloat(c.CUSTO_CONSULTA).toFixed(2).replace('.',',')}</td>`; if (!restricaoCustoApi) tr += `<td class="text-warning fw-bold">R$ ${parseFloat(c.CUSTO_V8).toFixed(2).replace('.',',')}</td>`; tr += `<td class="fw-bold ${ativo ? 'text-success' : 'text-muted'} fs-6">R$ ${parseFloat(c.SALDO).toFixed(2).replace('.',',')}</td>`; tr += `<td class="text-center"><span class="badge ${ativo ? 'bg-success' : 'bg-secondary'} px-2 py-1">${ativo ? 'ATIVO' : 'INATIVO'}</span></td>`; tr += `<td class="text-center"><button class="btn btn-sm btn-dark border-dark me-1" onclick="abrirModalEditarChave(${c.ID})"><i class="fas fa-edit"></i> Editar</button><button class="btn btn-sm ${ativo ? 'btn-outline-danger' : 'btn-outline-success'} border-dark" onclick="v8ToggleStatusChave(${c.ID}, '${c.STATUS}')" title="${ativo ? 'Desativar Robô' : 'Ativar Robô'}"><i class="fas fa-power-off"></i> ${ativo ? 'Desativar' : 'Ativar'}</button></td></tr>`; tb.innerHTML += tr; }); } else { tb.innerHTML = `<tr><td colspan="7" class="text-center py-4 text-danger fw-bold">Erro: ${r.msg}</td></tr>`; } }
    async function v8ToggleStatusChave(id, statusAtual) { const novoStatus = statusAtual === 'ATIVO' ? 'INATIVO' : 'ATIVO'; const confirmMsg = novoStatus === 'INATIVO' ? 'Desativar este robô? Ele não aparecerá mais no Hub.' : 'Reativar este robô?'; if (!confirm(confirmMsg)) return; const res = await v8Req('v8_api.ajax.php', 'toggle_status_chave', { id, status: novoStatus }, false); if (res.success) { v8CarregarClientes(); v8CarregarDadosIniciais(); crmToast(novoStatus === 'ATIVO' ? '✅ Robô ativado!' : '🔴 Robô desativado!', novoStatus === 'ATIVO' ? 'success' : 'warning'); } else { crmToast('Erro: ' + res.msg, 'error'); } }
    ...
    async function v8CarregarFila(offset = 0, append = false) {
        ...
        if(r.success) {
            ...
            r.data.forEach(x => {
                ...
                if (x.STATUS_V8 === 'ERRO-MARGEM' || x.STATUS_V8 === 'ERRO LEITURA MARGEM' || x.STATUS_CONFIG_ID === 'ERRO CONSIG_ID' || x.STATUS_CONFIG_ID === 'ERRO CACHE V8') { let detalheErroConf = x.OBS_CONFIG_ID || msgErroLimpa; colConf = `${badgeFonteMargem}<span class="badge bg-danger mb-1">REJEITADO</span><br><small class="text-danger fw-bold d-block" style="font-size:9px;" title="${detalheErroConf}">${detalheErroConf}</small>`; } else if (x.STATUS_V8 === 'AGUARDANDO MARGEM' || x.STATUS_V8 === 'AGUARDANDO V8 MARGEM E PRAZOS') { 
