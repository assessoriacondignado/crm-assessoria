...
// ✅ ROTA CORRIGIDA SEGUNDO A DOCUMENTAÇÃO OFICIAL (channel no singular)
$url_disparo = "https://api.gptmaker.ai/v2/channel/{$GPT_CHANNEL_ID}/start-conversation";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['telefone'])) {
    ...
    $mensagem_ia = "Fala chefe! 🚀 Teste de disparo ativo via Formulário Web concluído com sucesso. A nova rota está voando!";

    // ✅ CHAVES DO JSON CORRIGIDAS SEGUNDO A DOCUMENTAÇÃO (phone e message)
...
<head>
    ...
    <style>
        ...
        .container { background-color: #fff; padding: 30px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); width: 100%; max-width: 500px; }
        ...
        button { width: 100%; background-color: #007bff; color: white; padding: 12px; border: none; border-radius: 5px; font-size: 16px; cursor: pointer; font-weight: bold; transition: background-color 0.3s; }
