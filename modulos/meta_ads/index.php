...
<script>
    ...
    async function iniciarEnvioGlobal() {
        ...
        if(!tplId) return crmToast("Selecione um modelo de mensagem primeiro!", "info", 5000);
        ...
        if (dadosUnicos) {
            ...
            if(botoes.length === 0) { btnIniciar.disabled = false; btnCancelar.disabled = false; return crmToast("Nenhum anúncio ATIVO na tela!", "info", 5000); }
    ...
    async function excluirTemplate(id) { if(!confirm("Certeza que deseja excluir este modelo de mensagem?")) return; const r = await metaReq('excluir_template', { id: id }); crmToast(r.msg, r.success === false ? "error" : "info"); carregarTemplates(); }

    document.getElementById('formTemplate').addEventListener('submit', async function(e) { e.preventDefault(); const dados = { id: document.getElementById('tpl_id').value, nome: document.getElementById('tpl_nome').value, local: document.getElementById('tpl_local').value, funcao: document.getElementById('tpl_funcao').value, conteudo: document.getElementById('tpl_conteudo').value }; const r = await metaReq('salvar_template', dados); crmToast(r.msg, r.success === false ? "error" : "info"); if(r.success) { bootstrap.Modal.getInstance(document.getElementById('modalTemplate')).hide(); carregarTemplates(); carregarAgendamentos(); } });
    ...
    document.getElementById('formAgendamento').addEventListener('submit', async function(e) { e.preventDefault(); let diasSelecionados = []; document.querySelectorAll('.chk-dia:checked').forEach(c => diasSelecionados.push(c.value)); if (diasSelecionados.length === 0) return crmToast("Selecione pelo menos um dia da semana!", "info", 5000); const dados = { id: document.getElementById('agen_id').value, template_id: document.getElementById('agen_template_id').value, horarios: document.getElementById('agen_horarios').value, dias: diasSelecionados.join(',') }; const r = await metaReq('salvar_agendamento', dados); crmToast(r.msg, r.success === false ? "error" : "info"); if (r.success) { bootstrap.Modal.getInstance(document.getElementById('modalAgendamento')).hide(); carregarAgendamentos(); } });
    ...
    async function salvarRegistroDiario() { const vId = document.getElementById('reg_vinculo_id').value; const saldoDisponivel = document.getElementById('reg_saldo_disponivel').value; const st = document.getElementById('reg_status').value; const obs = document.getElementById('reg_obs').value; const whats = document.getElementById('reg_enviar_whats').checked ? 1 : 0; const tplId = document.getElementById('reg_template_select').value; const destino = document.getElementById('reg_destino').value; if (whats === 1 && !tplId) return crmToast("Por favor, selecione um Modelo de Mensagem ou crie um em Configurações.", "info", 5000); const btn = event.currentTarget; btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Processando...'; const r = await metaReq('salvar_registro_diario', { vinculo_id: vId, status: st, observacao: obs, enviar_whats: whats, template_id: tplId, destino: destino, saldo_disponivel: saldoDisponivel }); crmToast(r.msg, r.success === false ? "error" : "info"); btn.disabled = false; btn.innerHTML = '<i class="fas fa-check me-1"></i> Gravar Lançamento'; if(r.success) { document.getElementById('reg_obs').value = ''; const tb = document.getElementById('tbody_registros'); tb.innerHTML = "<tr><td colspan='4' class='text-center py-4'><i class='fas fa-spinner fa-spin'></i> Atualizando histórico...</td></tr>"; const r2 = await metaReq('carregar_registros_diarios', { vinculo_id: vId }); if(r2.success) { tb.innerHTML = ""; r2.data.forEach(reg => { let badgeWhats = reg.MSG_ENVIADA == 1 ? '<span class="badge bg-success"><i class="fas fa-check"></i> Sim</span>' : '<span class="badge bg-secondary"><i class="fas fa-times"></i> Não</span>'; tb.innerHTML += `<tr class="border-bottom border-dark"><td class="text-muted fw-bold">${reg.DATA_REG_BR}</td><td><span class="badge bg-dark border border-secondary">${reg.STATUS_DIARIO}</span></td><td class="text-start">${reg.OBSERVACAO || '-'}</td><td>${badgeWhats}</td></tr>`; }); } } }
    ...
    async function salvarConfig() { const t = document.getElementById('cfg_meta_token').value; const r = await metaReq('salvar_config', { meta_token: t }); crmToast(r.msg, r.success === false ? "error" : "info"); }
    ...
    async function salvarVinculo() { const c = document.getElementById('vinculo_cliente').value; const id = document.getElementById('vinculo_ad_id').value; const acc_id = document.getElementById('vinculo_ad_account_id').value; if(!c || !id) return crmToast("Preencha o cliente e o ID do Anúncio!", "info", 5000); const r = await metaReq('salvar_vinculo', { cpf_cliente: c, ad_id: id, ad_account_id: acc_id }); crmToast(r.msg, r.success === false ? "error" : "info"); document.getElementById('vinculo_ad_id').value = ''; document.getElementById('vinculo_ad_account_id').value = ''; carregarRelatorio(); }
    ...
    async function excluirVinculo(id) { if(!confirm("Tem certeza que deseja remover este anúncio do painel?")) return; const r = await metaReq('excluir_vinculo', { id: id }); crmToast(r.msg, r.success === false ? "error" : "info"); carregarRelatorio(); }
    ...
    async function salvarDescricao() { const id = document.getElementById('desc_id').value; const texto = document.getElementById('desc_texto').value; const r = await metaReq('salvar_descricao', { id: id, descricao: texto }); crmToast(r.msg, r.success === false ? "error" : "info"); bootstrap.Modal.getInstance(document.getElementById('modalDescricao')).hide(); carregarRelatorio(); }
    async function sincronizarLinha(id) { if(!confirm("Atualizar nomes desta conta na Meta agora?")) return; const r = await metaReq('sincronizar_linha', { id: id }); crmToast(r.msg, r.success === false ? "error" : "info"); carregarRelatorio(); }
    async function sincronizarMetaGlob() { const btn = event.currentTarget; btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Sincronizando...'; const r = await metaReq('sincronizar_dados_meta'); crmToast(r.msg, r.success === false ? "error" : "info"); btn.disabled = false; btn.innerHTML = '<i class="fas fa-sync-alt me-1"></i> Sincronização Global'; carregarRelatorio(); }
    ...
    async function carregarRelatorio() {
        ...
        if (filtro === 'custom') { const di = document.getElementById('data_inicio').value; const df = document.getElementById('data_fim').value; if(!di || !df) return crmToast("Preencha as datas!", "info", 5000); filtro = `${di}|${df}`; dataReferenciaTexto = `De ${di.split('-').reverse().join('/')} até ${df.split('-').reverse().join('/')}`; }
