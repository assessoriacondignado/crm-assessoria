...
<div class="modal fade" id="modalIncluirCampanha" tabindex="-1">
    <div class="modal-dialog modal-sm modal-dialog-centered">
        ...
        <div class="px-3 pb-3 bg-light border-top border-dark" id="boxCampanhasVinculadas" style="display:none;">
            <p class="small fw-bold text-uppercase text-dark mb-1 mt-2" style="font-size:0.70rem;"><i class="fas fa-link me-1"></i> Campanhas vinculadas:</p>
            <div id="listaCampanhasVinculadas"></div>
        </div>
    </div>
</div>
<?php endif; ?>
...
// Campanhas vinculadas ao cliente no modal Incluir na Campanha
(function() {
    const modalEl = document.getElementById('modalIncluirCampanha');
    if (!modalEl) return;
    modalEl.addEventListener('shown.bs.modal', function() {
        const box   = document.getElementById('boxCampanhasVinculadas');
        const lista = document.getElementById('listaCampanhasVinculadas');
        if (!box || !lista) return;
        lista.innerHTML = '<div class="text-muted small">Carregando...</div>';
        box.style.display = 'block';

        const cpf = '<?= $cpf_selecionado ?>';
        const fd  = new FormData();
        fd.append('acao_rapida', 'campanhas_do_cliente');
        fd.append('cpf', cpf);
        fetch('', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(r => {
            if (!r.success || !r.campanhas || r.campanhas.length === 0) {
                lista.innerHTML = '<div class="text-muted small" style="font-size:0.70rem;">Nenhuma campanha vinculada.</div>';
                return;
            }
        ...
        .catch(() => {
            lista.innerHTML = '<div class="text-danger small">Erro ao carregar.</div>';
        });
    });
})();
